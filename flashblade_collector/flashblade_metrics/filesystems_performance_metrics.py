...
class FilesystemsPerformanceMetrics():
    ...
    def _latency(self):
        ...
        for f in self.fb.get_nfs_filesystems_performance():
            self.latency.add_metric(['nfs', f.name, 'read'],
                                    f.usec_per_read_op)
            self.latency.add_metric(['nfs', f.name, 'write'],
                                    f.usec_per_write_op)
            self.latency.add_metric(['nfs', f.name, 'other'],
                                    f.usec_per_other_op)
    ...
    def _ops_size(self):
        ...
        for f in self.fb.get_nfs_filesystems_performance():
            ...
            self.ops_size.add_metric(['nfs', f.name, 'write'],
                                     f.bytes_per_write)
    ...
    def _throughput(self):
        ...
        for f in self.fb.get_nfs_filesystems_performance():
            self.throughput.add_metric(['nfs', f.name, 'read'],
                                       f.read_bytes_per_sec)
            self.throughput.add_metric(['nfs', f.name, 'write'],
                                       f.write_bytes_per_sec)
